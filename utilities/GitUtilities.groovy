...
def getCurrentGitDetachedBranch(String gitDir) {
	...
	if (gitBranchesArray.count {it.contains(origin)}  > 1 ) {
		String warningMsg = "*! (GitUtils.getCurrentGitDetachedBranch) Warning obtaining branch name for ($gitDir). Multiple references point to the same commit. ($gitBranchArr)"
		println(warningMsg)
		updateBuildResult(warningMsg:warningMsg)
	}
